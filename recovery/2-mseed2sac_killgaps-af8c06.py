        ...
        for ifil, path in enumerate(pathlist):
            ...
            for i in np.arange(0, 3):
                ...
                t = np.linspace(st[i].stats.starttime.timestamp - dt,
                    ...
                    st[i].stats.npts)
                plt.plot(t, st[i].data, color=cmap.to_rgba(i+1))
                # if i == 0:
                #     plt.plot(t-(st[1].stats.starttime-st[0].stats.endtime), st[i].data, '--', color=(1,0,0), linewidth=0.5)
                plt.ylim(3*np.std(st[i].data), -3*np.std(st[i].data))
                plt.xlim(0, 3*24*60*60)
                plt.title(str(st[0].stats.starttime)+'  to  '+str(st[2].stats.endtime)+':  total time shift = '+str(np.round(dt_totalshift_sec/60/60,3))+'hours')
                ...
                # if i == 0:
                #     plt.plot(t-(st[1].stats.starttime-st[0].stats.endtime), st[i].data, '--', color=(1,0,0), linewidth=0.5)
                plt.ylim(1*np.std(st[i].data), -1*np.std(st[i].data))
                plt.xlim(st[0].stats.endtime.timestamp-dt-t_zoom, st[1].stats.starttime.timestamp-dt+t_zoom)
                ...
                #     plt.plot(t+(st[1].stats.starttime-st[0].stats.endtime), st[i].data, '--', color=(1,0,0), linewidth=0.5)
                plt.ylim(1*np.std(st[i].data), -1*np.std(st[i].data))
                plt.xlim(st[1].stats.endtime.timestamp-dt-t_zoom, st[2].stats.starttime.timestamp-dt+t_zoom)
            plt.show()
            figdir = './figs_killgaps/'+sta+'/'
            if 1:
                if not os.path.exists(figdir):
                    os.makedirs(figdir)
                plt.savefig(figdir+sta+'.'+ch+'_'+comp+'.'+oyr+'.'+ojday+'.'+ohr+'.'+omn+'.'+osec+'.pdf')
            plt.close()

...
for ista,sta in enumerate(inventory.station):
    ...
    for ch in chs:
        ...
        for ifil, path in enumerate(pathlist):
            ...
            dt_strt_gap_sec[ifil] = dt_strt_gap
            dt_end_gap_sec[ifil] = dt_end_gap
            # print(dt_strt_gap)
            ...
            nsec_mseed_actual[ifil] = st[1].stats.npts / st[1].stats.sampling_rate
            nsec_mseed_apparent[ifil] = st[1].stats.starttime - st[0].stats.endtime
            jdaynum_mseed[ifil] = ojday
            print('Mseed apparent length: ' + str(nsec_mseed_apparent[ifil]) + 's ; Mseed actual length: ' + str(nsec_mseed_actual[ifil]))
            ...
            dt_totalshift_sec_save[ifil] = dt_totalshift_sec
            ...
            for i in np.arange(0, 3):
                ...
                ax3.set(title='Mseed length: apparent='+str(nsec_mseed_apparent[ifil])+'s; actual='+str(nsec_mseed_actual[ifil])+'s')
...
#                     st.remove_response(output="DISP", zero_mean=True, taper=True, taper_fraction=0.05, pre_filt=[0.001, 0.005, sr/3, sr/2], water_level=60)
            ...
            fig.clear()
            plt.close(fig)
        dt_totalshift_sec_save_ch[ch] = dt_totalshift_sec_save
        nsec_mseed_actual_ch[ch] = nsec_mseed_actual
        nsec_mseed_apparent_ch[ch] = nsec_mseed_apparent
        jdaynum_mseed_ch[ch] = jdaynum_mseed
        jdaynum_sac_ch[ch] = jdaynum_sac
        dt_strt_gap_sec_ch[ch] = dt_strt_gap_sec
        dt_end_gap_sec_ch[ch] = dt_end_gap_sec
